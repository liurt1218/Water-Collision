...
def init_fluid_and_rigid():
    ...
    for b in range(n_rigid_bodies):
        center = ti.Vector([0.5 + 0.2 * b, 0.5 + 0.3 * b, 0.5])
        rb_pos[b] = center
...
def main():
    ...
    os.makedirs("frames/fall", exist_ok=True)
    ...
    for frame in range(n_frames):
        ...
        fname = f"frames/fall/frame_{frame:04d}.png"
        window.save_image(fname)
        print("saved", fname)
    os.system(
        "ffmpeg -framerate 30 -i frames/fall/frame_%04d.png -c:v libx264 -pix_fmt yuv420p fall.mp4 -y"
    )
